...
| `emulator_debug_tags` | Comma-separated list of emulator debug tags (e.g. `init,avd,kernel` or `all`). Passed to the emulator as `-debug <tags>`.  When this input is non-empty, the emulator host process stdout/stderr is saved to the path exported as `$BITRISE_EMULATOR_HOST_LOG`, and the device-side logcat stream is captured via `-logcat-output` to the path exported as `$BITRISE_EMULATOR_DEVICE_LOGCAT_LOG`. Both files are written to `$BITRISE_DEPLOY_DIR` and deployed as build artifacts. Logs are preserved even if the device never becomes reachable via `adb`.  Leave empty to disable. Run `emulator -help-debug-tags` locally to see the full list of available tags. |  |  |
| `device_logcat_tags` | Space- or comma-separated logcat filters in `componentName:logLevel` format, passed to the emulator as `-logcat <tags>`.  `componentName` is either `*` (wildcard) or a component name such as `ActivityManager` or `GSM`. `logLevel` is one of: `v` (verbose), `d` (debug), `i` (informative), `w` (warning), `e` (error), `s` (silent).  Example: `*:s GSM:i` — suppresses all logs except GSM at informative level.  Leave empty to disable. See `adb logcat --help` for more information. |  |  |
</details>
...
| `BITRISE_EMULATOR_HOST_LOG` | Path to the emulator process stdout/stderr log file. Only set when `emulator_debug_tags` is non-empty. |
| `BITRISE_EMULATOR_DEVICE_LOGCAT_LOG` | Path to the device-side logcat log file captured via `-logcat-output`. Only set when `emulator_debug_tags` is non-empty. |
</details>
