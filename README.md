...
| `debug_tags` | Comma-separated list of emulator debug tags (e.g. `init,avd,kernel` or `all`). Passed to the emulator as `-debug <tags>`.  When this input is non-empty, the emulator is also started with `-logcat-output` pointing to `$BITRISE_DEPLOY_DIR/<emulator_id>_device.log`, so the device-side logcat stream is captured to a file deployed as a build artifact. The file is written by the emulator process directly, so logs are preserved even if the device never becomes reachable via `adb`.  Leave empty to disable. Run `emulator -help-debug-tags` locally to see the full list of available tags. |  |  |
</details>
